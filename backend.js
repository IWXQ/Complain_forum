...
class Bynorth {
  ...
  addPost(content, hash, name) {
    if (content === null || content === '') return {'error': 0};
  ...
  addMessage(blogId, content, hash, name) {
    ...
    let r = {'time': time, 'messageId': messageId} console.log(r);
    return r;
  }
  getPost(option) {
    if (option === 0) {
  ...
  blogLikeDislike(hash, blogId, likeDislike) {
    ...
    if (!userStatus) {
      ...
      userStatus = new whetherLikeOrDislike({'status': st});
      if (likeDislike) {
        b.like += 1;
      } else {
        b.dislike += 1;
      ...
      if (likeDislike && userStatus.status === 2) {
        ...
        if (likeDislike) {
          if (userStatus.status !== 0) b.dislike -= 1;
          b.like += 1;
          userStatus.status = 2;
        } else {
          if (userStatus.status !== 0) b.like -= 1;
          b.dislike += 1;
          userStatus.status = 1;
        }
